                       ...
                       unsigned char length, unsigned char * data)
{
    ...
    HAL_StatusTypeDef ret_V = HAL_I2C_Master_Transmit(&hi2c3, slave_addr << 1, data_for_transmit, (length + 1), 0xff);

    if (ret_V != HAL_OK)
    {
        I2C_ClearBusyFlagErratum(&hi2c3, 10);
    }
...
int stm32_i2c_read(unsigned char slave_addr, unsigned char reg_addr,
                       unsigned char length, unsigned char * data)
{
    HAL_StatusTypeDef ret_V = HAL_I2C_Master_Transmit(&hi2c3, slave_addr << 1, &reg_addr, 1, 0xff);
    if (ret_V != HAL_OK)
    {
        I2C_ClearBusyFlagErratum(&hi2c3, 10);
        return ret_V;
    ...
    ret_V = HAL_I2C_Master_Receive(&hi2c3, slave_addr << 1, data, length, 0xff);
