...
| 4 | POSITION | 大位移使用位置环 + 速度环 + 电流环三级级联；距目标 `15°` 内切换为带速度阻尼的受限 Iq 位置保持，受外力拨动后会自动回正 |
...
- 各环目标值复用已有的 PID 写入命令：电流环 Id/Iq 用 `ID`/`IQ`，速度环用 `SPD`，位置环用 `POS`
  （见下方 PID 帧与 `$WPID` 命令）。主 BLDC 的 `POS` target 使用 `0~360°` 机械角，`0°` 与 `360°`
  是同一位置；固件内部转换为 rad 并按最短路径归一化，跨零位不会反转。`$PID,POS` 回读也使用度。
...
- 切入 `SPEED` 模式后，若速度目标非零，固件先按目标方向施加 `0.5 A` Iq、持续 300 ms，使转子
  ...
  不会施加该起动电流。
- `POSITION` 模式在目标 `±1°` 死区外、目标附近 `±15°` 范围内使用 `0.7~2.5 A` 的受限保持
  电流；长时间保持时电机可能发热，完成定位后应切回 `IDLE`。
...
| `0x05` | 调节位置环 PID/target | 见下 | 步进档位 | 同 `0x01`，target 步进单位为度 |
