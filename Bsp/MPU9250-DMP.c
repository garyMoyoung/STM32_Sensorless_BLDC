...
/**
 * @brief 限幅函数，返回value在[min, max]区间内的值
 */
...
inv_error_t MPU9250_begin(void)
{
	...
	printf("MPU9250 init result: %d\n", result);
	if (result)
		return result;

	mpu_set_bypass(1); // Place all slaves (including compass) on primary bus
	printf("MPU9250 set bypass\n");
	MPU9250_setSensors(INV_XYZ_GYRO | INV_XYZ_ACCEL | INV_XYZ_COMPASS);
	printf("MPU9250 set sensors\n");
...
void MPU9250_computeEulerAnglesWithMag(bool degrees)
{
    // 1. 先用DMP四元数算出pitch/roll
    float dqw = MPU9250_qToFloat(qw, 30);
    float dqx = MPU9250_qToFloat(qx, 30);
    float dqy = MPU9250_qToFloat(qy, 30);
    float dqz = MPU9250_qToFloat(qz, 30);
    ...
    // 磁力计倾斜补偿
    float mag_x = mx_f * cos(pitch) + mz_f * sin(pitch);
    float mag_y = mx_f * sin(roll) * sin(pitch) + my_f * cos(roll) - mz_f * sin(roll) * cos(pitch);

    // 3. 用补偿后的磁力计数据算yaw
    yaw_inside = atan2(-mag_y, mag_x);

    if (degrees) {
        pitch_inside *= (180.0 / PI);
        roll_inside  *= (180.0 / PI);
        yaw_inside   *= (180.0 / PI);
        if (pitch_inside < 0) pitch_inside += 360.0;
        if (roll_inside < 0) roll_inside += 360.0;
        if (yaw_inside < 0) yaw_inside += 360.0;
    }
}
